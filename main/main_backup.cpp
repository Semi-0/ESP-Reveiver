...
void centralized_logging_handler(const Event& e, void* user) {
    ...
    switch (e.type) {
        ...
        case TOPIC_MQTT_CONNECT_FAILED:
            event_name = "MQTT Connect Failed";
            log_message = "MQTT connection attempt failed";
            is_error = true;
            SystemStateManager::updateMqttState(false);
            SystemStateManager::incrementErrorCount();
            break;
...
static bool mdns_query_worker(void** out) {
    ESP_LOGI("MDNS_WORKER", "=== MDNS QUERY WORKER STARTED ===");
    ESP_LOGI("MDNS_WORKER", "Starting mDNS query for service: %s", MDNS_SERVICE_TYPE);
    ...
    if (err != ESP_OK) {
        ESP_LOGE("MDNS_WORKER", "mDNS query failed with error: %s", esp_err_to_name(err));
        char* host = strdup("10.0.1.1");  // Try the gateway IP instead
        ESP_LOGI("MDNS_WORKER", "Using fallback IP due to mDNS error: %s", host);
        *out = host;
        return true;
    }
    
    if (results == nullptr) {
        ESP_LOGW("MDNS_WORKER", "mDNS query returned null results, using fallback IP");
    ...
    while (r) {
        result_count++;
        ESP_LOGI("MDNS_WORKER", "Result %d: instance_name=%s, hostname=%s", 
                 result_count, r->instance_name ? r->instance_name : "null", 
                 r->hostname ? r->hostname : "null");
        
        // Check if this is our target service (or any MQTT service if we can't find the specific one)
        if (r->instance_name && (strcmp(r->instance_name, MDNS_SERVICE_NAME) == 0 || strstr(r->instance_name, "mqtt") != nullptr)) {
            if (r->hostname) {
                // Check if hostname is already an IP address
                if (strchr(r->hostname, '.') != nullptr && isdigit(r->hostname[0])) {
                    // It looks like an IP address, use it directly
                    ESP_LOGI("MDNS_WORKER", "Hostname appears to be IP address: %s", r->hostname);
                    char* ip_address = strdup(r->hostname);
                    *out = ip_address;
                    
                    mdns_query_results_free(results);
                    ESP_LOGI("MDNS_WORKER", "Final result: %s", static_cast<const char*>(*out));
                    return true;
                } else {
                    ...
                    if (r->addr && r->addr->addr.type == ESP_IPADDR_TYPE_V4) {
                        ...
                        ESP_LOGI("MDNS_WORKER", "Found IP from service record: %s", ip);
                        *out = strdup(ip);
                        ...
                        ESP_LOGI("MDNS_WORKER", "Querying A record for hostname: %s", r->hostname);
                        esp_ip4_addr_t addr;
                        ...
                        if (addr_err == ESP_OK) {
                            ...
                            ESP_LOGI("MDNS_WORKER", "Resolved hostname %s to IP: %s", r->hostname, ip);
                            *out = strdup(ip);
                            ...
                            ESP_LOGW("MDNS_WORKER", "Failed to resolve hostname %s, using .local fallback", r->hostname);
                            std::string full_hostname = std::string(r->hostname) + ".local";
                            char* hostname = strdup(full_hostname.c_str());
                            ESP_LOGI("MDNS_WORKER", "Using fallback hostname: %s", hostname);
                            *out = hostname;
    ...
    ESP_LOGW("MDNS_WORKER", "No matching service found, using hardcoded fallback IP");
    char* host = strdup("10.0.1.1");  // Try the gateway IP instead
    ESP_LOGI("MDNS_WORKER", "Using fallback IP: %s", host);
    mdns_query_results_free(results);
    *out = host;
    ESP_LOGI("MDNS_WORKER", "Final result: %s", static_cast<const char*>(*out));
    return true;
...
static bool mqtt_connection_worker_with_event(const Event& trigger_event, void** out) {
    ESP_LOGI("MQTT_WORKER_EVENT", "=== MQTT WORKER STARTED ===");
    ESP_LOGI("MQTT_WORKER_EVENT", "Received event with ptr: %p", trigger_event.ptr);
...
extern "C" void app_main(void) {
    ...
    G.when(TOPIC_WIFI_CONNECTED,
        G.async_blocking("mdns-query", mdns_query_worker,
            ...
            [](const Event& e, IEventBus& bus) {
                ...
                ESP_LOGI("MDNS_WORKER", "Flow 1 - Found hostname: %s", hostname ? hostname : "null");
                char* hostname_copy = hostname ? strdup(hostname) : nullptr;
                ESP_LOGI("MDNS_WORKER", "Flow 1 - Publishing TOPIC_MDNS_FOUND with hostname: %s", hostname_copy ? hostname_copy : "null");
                ESP_LOGI("MDNS_WORKER", "Flow 1 - About to publish event with ptr: %p", hostname_copy);
                Event event{TOPIC_MDNS_FOUND, 0, hostname_copy, free};
                ESP_LOGI("MDNS_WORKER", "Flow 1 - Event created with ptr: %p", event.ptr);
                bus.publish(event);
                ESP_LOGI("MDNS_WORKER", "Flow 1 - Event published");
            },
    ...
    G.when(TOPIC_RETRY_RESOLVE,
        G.async_blocking("mdns-query", mdns_query_worker,
            [](const Event& e, IEventBus& bus) {
                const char* hostname = static_cast<const char*>(e.ptr);
                ESP_LOGI("MDNS_WORKER", "Flow 3 - Found hostname: %s", hostname ? hostname : "null");
                char* hostname_copy = hostname ? strdup(hostname) : nullptr;
                ESP_LOGI("MDNS_WORKER", "Flow 3 - Publishing TOPIC_MDNS_FOUND with hostname: %s", hostname_copy ? hostname_copy : "null");
    ...
    G.when(TOPIC_MDNS_FOUND,
        FlowGraph::seq(
            ...
            G.async_blocking_with_event("mqtt-connect", mqtt_connection_worker_with_event,
                ...
                FlowGraph::seq(
                    FlowGraph::publish(TOPIC_MQTT_CONNECT_FAILED, 0, nullptr),
    ...
    // Flow 5.5: MQTT connection failed – schedule retry (separate from disconnection)
    G.when(TOPIC_MQTT_CONNECT_FAILED,
        FlowGraph::tap([](const Event& e) {
            schedule_reconnect();
        })
    );
