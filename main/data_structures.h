...
// Pin command data for event bus
struct PinCommandData {
    int pin;
    int value;
    std::string description;
    
    PinCommandData(int p, int v, const std::string& desc) 
        : pin(p), value(v), description(desc) {}
};
...
struct DeviceCommandResult {
    ...
    static DeviceCommandResult success_result(const std::string& action, int pin, int value) {
        return DeviceCommandResult(pin, value, true, action, "");
    }
    
    static DeviceCommandResult failure_result(const std::string& error, int pin = 0) {
        return DeviceCommandResult(pin, 0, false, "", error);
    }
};
