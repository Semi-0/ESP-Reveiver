    ...
    ListenerHandle when(uint16_t topic, const Flow& f) {
        ...
        auto h = bus_.subscribe(
            [](const Event& e, void* u) {
                auto* self = static_cast<FlowGraph*>(u);
                self->(*self->current_)(e, self->bus_); // never used; replaced below
            }, this, bit(topic)
        );
    ...
    Flow async_blocking(const char* name,
        ...
        return [=, this](const Event& trigger, IEventBus& /*bus*/) {
            ...
            xTaskCreate([](void* arg){
                struct TaskPack { std::function<bool(void**)> fn; AsyncCtx* ctx; };
...
private:
    ...
    // Dummy to satisfy early lambda (not used)
    Flow* current_ = nullptr;
};
